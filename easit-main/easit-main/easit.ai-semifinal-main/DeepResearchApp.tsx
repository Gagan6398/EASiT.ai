...
const PROGRESS_STEPS = [
    { icon: Search, label: 'Initializing deep search...', color: 'text-[#CFA54D]' },
    { icon: Database, label: 'Retrieving verified data...', color: 'text-cyan-400' },
    { icon: Brain, label: 'Synthesizing knowledge...', color: 'text-[#B8860B]' },
    { icon: ShieldCheck, label: 'Finalizing research report...', color: 'text-green-400' },
];
            ...
            <div className="bg-white shadow-sm border border-gray-100 rounded-3xl rounded-tl-sm p-6 w-full max-w-2xl">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-500">Step {step + 1} of {PROGRESS_STEPS.length} — Iterative research pipeline</p>
                    </div>
                </div>
                {/* Progress bar */}
...
const AgentCards: React.FC<{ metadata: ConsensusMetadata }> = ({ metadata }) => {
    ...
    if (!metadata.agentBreakdown || metadata.agentBreakdown.length === 0) {
        return null; // Do not show if no agent breakdown exists
    }
            ...
            {expanded && (
                <div className="grid grid-cols-2 gap-3 mt-3 animate-slide-up-fade-in">
                    {metadata.agentBreakdown.map((agent, i) => {
                        const config = AGENT_CONFIG[agent.agent as keyof typeof AGENT_CONFIG] || AGENT_CONFIG.gemini;
                        return (
            ...
            <header className="flex items-center justify-between p-4 border-b border-gray-100 bg-white shadow-md border-gray-100 backdrop-blur-md sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-bold tracking-[0.2em] text-[#CFA54D] uppercase">
                    ...
                    {results.length === 0 && !isResearching && (
                        <div className="flex flex-col items-center justify-center py-20 text-center opacity-70 animate-slide-up-fade-in">
                            ...
                            <p className="max-w-md text-gray-600">
                                Enter a complex query. The system will deploy the Deep Research Engine
                                with real-time streaming, search the web, and synthesize a rigorously validated,
            ...
            <div className="p-4 bg-cream-bg border-t border-gray-100 pb-8">
                ...
                <div className="text-center mt-4">
                    <p className="text-center text-xs text-gray-500 mt-6 pb-6">
                        Powered by Deep Research Architecture — Streaming + Search Grounding + Data Synthesis
                    </p>
                </div>
            </div>
        </div>
